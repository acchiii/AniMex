<header class="bg-white/80 dark:bg-[#0a0a0f]/80 backdrop-blur-lg border-b border-gray-200 dark:border-gray-800/60 sticky top-0 z-50 transition-colors duration-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16 gap-4">
            ...
            <nav class="hidden lg:flex items-center gap-6 flex-1 justify-center">
                <a href="{{ route('home') }}" class="text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 transition {{ request()->routeIs('home') ? 'text-purple-600 dark:text-purple-400' : '' }}">Home</a>
                <a href="{{ route('anime.index') }}" class="text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 transition {{ request()->routeIs('anime.index') ? 'text-purple-600 dark:text-purple-400' : '' }}">Browse</a>
                <a href="{{ route('anime.popular') }}" class="text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 transition {{ request()->routeIs('anime.popular') ? 'text-purple-600 dark:text-purple-400' : '' }}">Popular</a>
                <a href="{{ route('anime.schedule') }}" class="text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 transition {{ request()->routeIs('anime.schedule') ? 'text-purple-600 dark:text-purple-400' : '' }}">Schedule</a>
            </nav>

            {{-- Right: Search + Auth + Theme + Hamburger --}}
            <div class="flex items-center gap-3 shrink-0">
                <form method="GET" action="{{ route('anime.index') }}" class="relative">
                    ...
                    <input type="search" name="search" placeholder="Search..." value="{{ request('search') }}" class="w-36 sm:w-48 lg:w-56 bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-gray-700/50 rounded-full pl-9 pr-4 py-1.5 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition">
                </form>

                @auth
                    <div class="relative">
                        <button type="button" class="flex items-center gap-2" onclick="document.getElementById('user-dropdown').classList.toggle('hidden')">
                            <img src="{{ auth()->user()->getAvatarUrlAttribute() }}" alt="Avatar" class="w-8 h-8 rounded-full border border-gray-200 dark:border-gray-800 object-cover">
                        </button>
                        ...
                        <div id="user-dropdown" class="hidden absolute right-0 mt-3 w-56 bg-white/90 dark:bg-[#0a0a0f]/90 backdrop-blur-lg border border-gray-200 dark:border-gray-800/60 rounded-2xl shadow-lg overflow-hidden z-50">
                            ...
                            <a href="{{ route('profile.show') }}" class="block px-4 py-3 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-white/5">
                                <span class="mr-2">👤</span> Profile
                            </a>

                            @if(!auth()->user()->hasVerifiedEmail())
                                <form method="POST" action="{{ route('verification.resend') }}" class="block">
                                    @csrf
    ...
    <div id="mobile-menu" class="hidden fixed inset-0 top-16 lg:hidden z-50">
        ...
        <nav class="relative bg-white dark:bg-[#0a0a0f] border-t border-gray-200 dark:border-gray-800/60 px-4 py-5 space-y-1 max-h-[calc(100vh-4rem)] overflow-y-auto">
            <a href="{{ route('home') }}" onclick="toggleMobileMenu()" class="block px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 {{ request()->routeIs('home') ? 'bg-gray-100 dark:bg-white/5 text-purple-600 dark:text-purple-400' : '' }}">Home</a>
            <a href="{{ route('anime.index') }}" onclick="toggleMobileMenu()" class="block px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 {{ request()->routeIs('anime.index') ? 'bg-gray-100 dark:bg-white/5 text-purple-600 dark:text-purple-400' : '' }}">Browse</a>
            <a href="{{ route('anime.popular') }}" onclick="toggleMobileMenu()" class="block px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 {{ request()->routeIs('anime.popular') ? 'bg-gray-100 dark:bg-white/5 text-purple-600 dark:text-purple-400' : '' }}">Popular</a>
            <a href="{{ route('anime.schedule') }}" onclick="toggleMobileMenu()" class="block px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 {{ request()->routeIs('anime.schedule') ? 'bg-gray-100 dark:bg-white/5 text-purple-600 dark:text-purple-400' : '' }}">Schedule</a>
            @auth
                <a href="{{ route('profile.show') }}" onclick="toggleMobileMenu()" class="block px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5">👤 Profile</a>
            @else
