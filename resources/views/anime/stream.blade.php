    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="aspect-video bg-black rounded-lg overflow-hidden mb-4 relative" id="player-container">
            @if($episode->sources->count())
                ...
                <iframe id="embed-player" class="w-full h-full @if($episode->sources->first()->type !== 'embed') hidden @endif"
        ...
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $anime->title_english ?: $anime->title }}
                </h1>
                <p class="text-gray-600 dark:text-gray-400">Episode {{ $episode->number }}: {{ $episode->title }}</p>
            </div>
            <div class="flex items-center gap-2">
                @if($prevEpisode)
                    <a href="{{ route('anime.stream', ['slug' => $anime->slug, 'episodeNumber' => $prevEpisode->number]) }}"
                ...
                @endif
                @if($nextEpisode)
                    <a href="{{ route('anime.stream', ['slug' => $anime->slug, 'episodeNumber' => $nextEpisode->number]) }}"
                        class="px-4 py-2 bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 rounded-lg transition text-gray-900 dark:text-white">
                        Next →
                    </a>
                @endif
            ...
            <div class="mb-4">
                ...
                <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-1.5 max-h-48 overflow-y-auto p-2 bg-gray-100 dark:bg-gray-800 rounded-lg">
                    @foreach($episodes as $ep)
                        <a href="{{ route('anime.stream', ['slug' => $anime->slug, 'episodeNumber' => $ep->number]) }}"
...
@push('scripts')
    ...
    <script>
        ...
        function loadSource(index) {
            ...
            const proxyUrl = '{{ route("proxy.source", "") }}/' + src.id;
