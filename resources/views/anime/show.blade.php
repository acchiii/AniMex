...
<div class="relative">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <div class="flex-1">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    @endif
                    <span class="text-gray-400 text-sm">{{ $anime->type }}</span>
                    @if($anime->episodes_count)
                        <span class="text-gray-400 text-sm">{{ $anime->episodes_count }} episodes</span>
                    @endif
                </div>

                <h1 class="text-3xl font-bold text-white mb-2">{{ $anime->title_english ?: $anime->title }}</h1>
                @if($anime->title_english)
                    <p class="text-gray-400 mb-4">{{ $anime->title_japanese }}</p>
                @endif
                ...
                <div class="flex items-center gap-4 mb-4">
                    @if($anime->score)
                        <div class="flex items-center gap-1">
                            ...
                            <span class="text-white font-medium">{{ number_format($anime->score, 1) }}</span>
                        </div>
                    @endif

                    @auth
                        <button onclick="toggleFavorite({{ $anime->id }})" 
                            class="flex items-center gap-1 px-3 py-1 rounded {{ $isFavorite ? 'bg-red-600' : 'bg-gray-700' }} hover:bg-opacity-80 transition">
                        ...
                        <div class="flex items-center gap-1" id="rating-section">
                            ...
                            @endfor
                            @if($userRating)
                                <span class="text-gray-400 text-sm ml-2">Your rating: {{ $userRating->score }}</span>
                            @endif
                        </div>
                    @endauth
                </div>
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        @foreach($anime->genres as $genre)
                            <a href="{{ route('anime.genre', $genre->slug) }}" 
                                class="bg-gray-800 hover:bg-gray-700 px-3 py-1 rounded text-sm text-gray-300 hover:text-white transition">
                                {{ $genre->name }}
                            </a>
                        @endforeach
                    </div>
                ...
                <!-- Synopsis -->
                <p class="text-gray-300 mb-4">{{ $anime->synopsis }}</p>

                <!-- Studio -->
                @if($anime->studio)
                    <p class="text-gray-400 text-sm">Studio: <a href="{{ route('anime.studio', $anime->studio->slug) }}" class="text-purple-500 hover:text-purple-400">{{ $anime->studio->name }}</a></p>
                @endif
            </div>
        </div>
            ...
            <div class="mt-8">
                ...
                <div class="space-y-2">
                    @foreach($anime->episodes as $episode)
                        <a href="{{ route('anime.stream', ['slug' => $anime->slug, 'episodeSlug' => $episode->slug]) }}" 
                            class="flex items-center gap-4 p-3 bg-gray-900 hover:bg-gray-800 rounded-lg transition">
                            <span class="text-gray-400 w-12">EP {{ $episode->number }}</span>
                            <span class="flex-1 text-white">{{ $episode->title }}</span>
                            @if($episode->is_filler)
                                <span class="text-xs bg-yellow-600 text-white px-2 py-0.5 rounded">Filler</span>
                            @endif
                            @if($episode->sources->count())
                                <span class="text-xs text-gray-500">{{ $episode->sources->count() }} sources</span>
                            @endif
                        </a>
                    @endforeach
                </div>
        ...
        @if($related->count())
            <div class="mt-8">
                <h2 class="text-xl font-bold text-white mb-4">Related Anime</h2>
                    ...
                    @foreach($related as $item)
                        <a href="{{ route('anime.show', $item->slug) }}" class="group">
                            <div class="relative aspect-[3/4] rounded-lg overflow-hidden bg-gray-800">
                                @if($item->cover_image)
                                    <img src="{{ asset('storage/' . $item->cover_image) }}" alt="{{ $item->title }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-300">
                                @else
                            ...
                            <h3 class="mt-2 text-sm font-medium text-white group-hover:text-purple-400 truncate">{{ $item->title }}</h3>
                        </a>
                    @endforeach
                </div>
            </div>
